...
{
	NSData *paramsData = [paramsString dataUsingEncoding:NSASCIIStringEncoding allowLossyConversion:YES];
	NSString *paramsLength = [NSString stringWithFormat:@"%d", [paramsData length]];
	
	NSString *requestFullURL = [requestURL stringByAppendingString:requestName];
	NSMutableURLRequest *request = [[NSMutableURLRequest alloc] initWithURL:[NSURL URLWithString:requestFullURL]];
	...
	NSData *responseData = [NSURLConnection sendSynchronousRequest:request returningResponse:&response error:errorPointer];
	...
	//NSLog(@"NSDictionary: %@ \n", [XMLReader dictionaryForXMLData:responseData error:errorPointer]);
	
	return [XMLReader dictionaryForXMLData:responseData error:errorPointer];
}
...
- (void)parser:(NSXMLParser *)parser foundCharacters:(NSString *)string
{
    ...
    if (![textInProgress isEqualToString:@""])
        [textInProgress appendString:[string stringByTrimmingCharactersInSet:[NSCharacterSet characterSetWithCharactersInString:@"\n\t"]]];
	else
        textInProgress = [NSMutableString stringWithString:[string stringByTrimmingCharactersInSet:[NSCharacterSet characterSetWithCharactersInString:@"\n\t"]]];
}
